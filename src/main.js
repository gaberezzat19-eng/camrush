...
{id:'smile',icon:'◒',title:'Smile Blast',tag:'FACE',desc:'Smile to charge your blast and clear targets.',color:'pink',time:'45 SEC'},
{id:'pong',icon:'▰',title:'Hand Pong',tag:'HAND',desc:'Move your hand to control the paddle.',color:'lime',time:'ENDLESS'}];

app.innerHTML='<div class="shell"><header class="topbar"><a class="brand" href="./"><span class="brand-mark">◈</span><span>CAM<span>RUSH</span></span></a><div class="top-actions"><span class="pill"><i></i> CAMERA LOCAL</span><button class="icon-btn" id="installBtn" hidden>Install</button></div></header><main><section class="hero"><div class="eyebrow">CAMERA ARCADE <b>•</b> ZERO SIGN-UP</div><h1>Move. Play.<br><em>Rush.</em></h1><p>Four quick camera games. Your movements are the controller.</p><div class="hero-actions"><button class="primary" id="playBtn">PLAY NOW <span>→</span></button><span class="hint">Chrome / Edge / Safari</span></div></section><section class="games-head"><div><span class="section-kicker">THE ARCADE</span><h2>Pick your rush.</h2></div><div class="score-chip">BEST <strong id="bestScore">'+state.best+'</strong></div></section><section class="grid" id="gameGrid"></section><section class="trust"><div><span class="trust-icon">⌁</span><div><b>Processed on your device</b><small>Your camera feed is used locally by the game.</small></div></div><div><span class="trust-icon">⚡</span><div><b>Fast by design</b><small>Vision models load only when a game starts.</small></div></div><div><span class="trust-icon">＋</span><div><b>Installable</b><small>Add CamRush to your home screen.</small></div></div></section></main><footer><span>CAMRUSH</span><span>Four games. One camera.</span></footer></div><div class="modal hidden" id="gameModal"><div class="game-window"><div class="gamebar"><div><span id="gameTag"></span><b id="gameTitle"></b></div><div class="gamebar-right"><span id="gameScore">0</span><button id="closeGame">×</button></div></div><div class="stage-wrap"><canvas id="gameCanvas"></canvas><video id="camera" playsinline muted></video><div class="loading" id="loading"><div class="spinner"></div><b>Waking the camera…</b><small>Allow camera access to play.</small></div><div class="countdown hidden" id="countdown"></div><div class="game-tip" id="gameTip"></div></div><div class="game-footer"><span id="gameStatus">GET READY</span><button class="secondary" id="restartGame">RESTART</button></div></div></div><div class="toast hidden" id="toast"></div>';

const grid=document.querySelector('#gameGrid');
grid.innerHTML=games.map(g=>'<article class="card '+g.color+'" data-game="'+g.id+'"><div class="card-art"><div class="orb">'+g.icon+'</div><span class="game-time">'+g.time+'</span><span class="card-tag">'+g.tag+'</span></div><div class="card-copy"><h3>'+g.title+'</h3><p>'+g.desc+'</p><button class="play-card">PLAY <span>→</span></button></div></article>').join('');
...
async function gameSmile(){const {x,w,h}=canvas(),tr=await tracker('smile');let charge=0,ts=[],last=0,end=performance.now()+45000;document.querySelector('#gameStatus').textContent='SMILE TO CHARGE';function smiling(r){const c=r.faceBlendshapes?.[0]?.categories||[];const a=c.find(q=>q.categoryName==='mouthSmileLeft')?.score||0,b=c.find(q=>q.categoryName==='mouthSmileRight')?.score||0;return(a+b)/2>.42}function loop(now){if(now>end){finish();return}const r=tr.detectForVideo(state.video,now);drawVideo(x,w,h);const s=smiling(r);charge=Math.max(0,Math.min(100,charge+(s?2.4:-.8)));if(now-last>650){last=now;ts.push({x:Math.random()*.8+.1,y:Math.random()*.65+.18,r:18+Math.random()*10})}if(charge>=100){addScore(3);charge=0;ts=[]}for(const t of ts){x.beginPath();x.arc(t.x*w,t.y*h,t.r,0,7);x.strokeStyle='#fb7185';x.lineWidth=3;x.stroke()}x.fillStyle='rgba(255,255,255,.14)';x.roundRect(18,h-38,w-36,14,7);x.fill();x.fillStyle='#fb7185';x.roundRect(18,h-38,(w-36)*charge/100,14,7);x.fill();x.fillStyle='#fff';x.font='700 13px system-ui';x.fillText(s?'SMILE!':'SMILE TO CHARGE',18,28);state.raf=requestAnimationFrame(loop)}loop(performance.now())}
async function gamePong(){const {x,w,h}=canvas(),tr=await tracker('hand');let paddle=w/2,bx=w/2,by=h*.45,vx=4,vy=4;document.querySelector('#gameStatus').textContent='KEEP IT ALIVE';function loop(now){const r=tr.detectForVideo(state.video,now);drawVideo(x,w,h);const p=r.landmarks?.[0]?.[8];if(p)paddle+=(1-p.x)*w-paddle;paddle=Math.max(75,Math.min(w-75,paddle));bx+=vx;by+=vy;if(bx<12||bx>w-12)vx*=-1;if(by<12)vy=Math.abs(vy);if(by>h-55&&by<h-25&&Math.abs(bx-paddle)<90){vy=-Math.abs(vy)*1.02;addScore()}if(by>h+30){finish();return}x.beginPath();x.arc(bx,by,11,0,7);x.fillStyle='#67e8f9';x.shadowBlur=20;x.shadowColor='#67e8f9';x.fill();x.shadowBlur=0;x.fillStyle='#fff';x.roundRect(paddle-72,h-34,144,14,7);x.fill();x.fillStyle='#fff';x.font='700 13px system-ui';x.fillText('MOVE YOUR HAND',18,28);state.raf=requestAnimationFrame(loop)}loop(performance.now())}
async function startEngine(id){document.querySelector('#loading').classList.add('hidden');if(id==='catch')return gameCatch();if(id==='dodge')return gameDodge();if(id==='smile')return gameSmile();return gamePong()}
